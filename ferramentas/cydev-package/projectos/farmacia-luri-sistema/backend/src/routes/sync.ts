...
router.post('/batch', syncAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!branchId || !Array.isArray(operations)) {
    ...
    if (result.success) {
      return res.json({ success: true, batchId, synced: result.processedCount });
    }
    return res.status(422).json({ success: false, error: result.error });
  } catch (err: any) {
    res.status(500).json({ error: 'Erro interno no servidor de sync', details: err.message });
  }
});
