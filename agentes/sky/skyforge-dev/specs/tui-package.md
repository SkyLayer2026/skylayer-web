...
- Do not replace private `packages/opencode` imports with `@skyforge/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@skyforge/tui`.
...
Status: Completed. The private `@skyforge/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@skyforge/tui`.
...
- The package has no dependency on `opencode`, `@skyforge/cli`, or
  `@skyforge/core`.
...
primitives now live in `@skyforge/tui`. The legacy host consumes explicit
...
and build inputs through `@skyforge/tui/runtime`. Movable app, component,
...
`@skyforge/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@skyforge/tui`. Configured references resolve through the new
...
`@skyforge/tui`. The legacy host injects a narrow plugin host that retains
...
public `@skyforge/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@skyforge/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@skyforge/core`, or either
  executable package.
...
rg '@skyforge/core|packages/opencode|packages/cli' packages/tui
