...
- `@skyforge/effect-drizzle-sqlite`
...
1. Add `@skyforge/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@skyforge/effect-drizzle-sqlite` as a dependency of `packages/opencode`.
5. Port `packages/opencode/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus skyforge-specific transaction/post-commit context.
...
- Should `afterCommit` stay skyforge-specific until event publishing moves? Default answer: yes.
