import { createTuiBuildInfo, createTuiEnvironment, createTuiRenderer, run, type TuiHost } from "@skyforge/tui"
...
export async function runTui(transport: { url: string; headers: RequestInit["headers"] }) {
  ...
  const state = path.join(os.homedir(), ".local", "state", "skyforge")
