...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-skyforge-directory", "directory"],
    ["x-skyforge-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-skyforge-directory")
  next.headers.delete("x-skyforge-workspace")
  return next
}
...
export function createOpencodeClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-skyforge-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-skyforge-workspace": config.experimental_workspaceID,
    }
  }
