...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@skyforge/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import * as Log from "@skyforge/core/util/log"
import { errorData, errorMessage } from "@skyforge/tui/util/error"
import { isRecord } from "@skyforge/tui/util/record"
...
import { hasTheme, upsertTheme } from "@skyforge/tui/context/theme"
import { Global } from "@skyforge/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@skyforge/core/util/flock"
import { Flag } from "@skyforge/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@skyforge/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@skyforge/core/v1/config/plugin"
import { createCommandShim } from "@skyforge/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@skyforge/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".skyforge"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".skyforge", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".skyforge", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.SKYFORGE_PURE ? [] : pluginOrigins
    if (Flag.SKYFORGE_PURE && pluginOrigins.length) {
