...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from skyforge.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "skyforge"),
    ...(await Filesystem.findUp(["skyforge.json", "skyforge.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "skyforge"))
  }
