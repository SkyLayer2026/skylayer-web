...
import { TuiKeybind } from "@skyforge/tui/config/keybind"
...
import { TuiConfig } from "@skyforge/tui/config"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old skyforge.json shape) still get their settings applied.
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".skyforge") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".skyforge") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
