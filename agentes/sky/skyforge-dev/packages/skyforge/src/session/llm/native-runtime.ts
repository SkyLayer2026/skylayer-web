...
import type { LLMClientShape } from "@skyforge/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("skyforge"))
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @skyforge/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains skyforge-owned. The native runtime only adapts
      // the @skyforge/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
