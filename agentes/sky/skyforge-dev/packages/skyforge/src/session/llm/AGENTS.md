...
- `ai-sdk.ts` converts AI SDK `fullStream` parts into `@skyforge/llm` `LLMEvent`s. This is the default runtime path.
- `native-request.ts` converts opencode's normalized session input into a native `@skyforge/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    ai-sdk.ts               AI SDK fullStream -> @skyforge/llm LLMEvent adapter
    native-request.ts       opencode/AI SDK-shaped input -> @skyforge/llm LLMRequest
    native-runtime.ts       native runtime gate, tool bridge, and LLMClient handoff
```

Integration points:

- `../llm.ts` imports `LLMClient` from `@skyforge/llm/route`; native execution is the only path that calls it directly.
...
- `native-request.ts` is the only adapter file that should construct `LLM.request(...)`, `LLM.model(...)`, `Message.*`, `SystemPart`, `ToolCallPart`, `ToolResultPart`, or `ToolDefinition` values from `@skyforge/llm`.
- `ai-sdk.ts` and `native-runtime.ts` both emit `@skyforge/llm` `LLMEvent`s so downstream session processing does not care which runtime handled the request.
...
`native-runtime.ts` evaluates the gate and either bridges into `@skyforge/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays skyforge-owned in both branches; only request lowering and transport differ.
...
- Native execution currently supports OpenAI, skyforge-managed OpenAI-compatible, and Anthropic API-key paths backed by `@ai-sdk/openai`, `@ai-sdk/openai-compatible`, or `@ai-sdk/anthropic` catalog entries.
