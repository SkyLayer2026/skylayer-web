...
import type { TuiConfig } from "@skyforge/tui/config"
import { createTuiBuildInfo, createTuiEnvironment } from "@skyforge/tui/runtime"
...
export function resolveTuiRuntime(config: TuiConfig.Resolved) {
  return {
    environment: createTuiEnvironment({
      ...
      initialRoute: parseInitialRoute(process.env.SKYFORGE_ROUTE),
      paths: {
      ...
      editor: {
        ...
        port: parsePort(process.env.CLAUDE_CODE_SSE_PORT || process.env.SKYFORGE_EDITOR_SSE_PORT),
        zedTerminal: isZedTerminal(),
        zedDatabase: resolveZedDbPath(),
      },
      skipInitialLoading: Boolean(process.env.SKYFORGE_FAST_BOOT),
    }),
