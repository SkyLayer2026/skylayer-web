...
import type { TuiPlatform } from "@skyforge/tui/platform"
...
import { readJson, writeJsonAtomic } from "@skyforge/tui/util/persistence"
  ...
  return {
    ...
    themes: {
      async discover() {
        const directories = [
          Global.Path.config,
          ...(await Array.fromAsync(Filesystem.up({ targets: [".skyforge"], start: process.cwd() }))),
        ]
