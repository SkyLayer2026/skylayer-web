...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall skyforge and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g skyforge-ai",
      pnpm: "pnpm uninstall -g skyforge-ai",
      bun: "bun remove -g skyforge-ai",
      yarn: "yarn global remove skyforge-ai",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "skyforge-ai"],
      yarn: ["yarn", "global", "remove", "skyforge-ai"],
      brew: ["brew", "uninstall", "skyforge"],
      choco: ["choco", "uninstall", "skyforge"],
      scoop: ["scoop", "uninstall", "skyforge"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "skyforge", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".skyforge/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".skyforge/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".skyforge/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".skyforge"))
    ) {
      continue
    }
