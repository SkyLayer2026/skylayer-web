...
import { ConfigMCPV1 } from "@skyforge/core/v1/config/mcp"
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in skyforge.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .skyforge/ subdirectory too)
  const candidates = [path.join(baseDir, "skyforge.json"), path.join(baseDir, "skyforge.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".skyforge", "skyforge.json"), path.join(baseDir, ".skyforge", "skyforge.jsonc"))
  }
  ...
  // Default to skyforge.json if none exist
  return candidates[0]
}
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "skyforge-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
