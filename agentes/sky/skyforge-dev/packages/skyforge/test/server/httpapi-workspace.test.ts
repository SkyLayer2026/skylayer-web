...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-skyforge-directory", directory)
  ...
  it.live("proxies remote workspace HTTP requests with sanitized forwarding", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-skyforge-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-skyforge-workspace")
