...
const withoutWatcher = <A, E, R>(effect: Effect.Effect<A, E, R>) => {
  ...
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.SKYFORGE_EXPERIMENTAL_DISABLE_FILEWATCHER
      process.env.SKYFORGE_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
      return previous
    }),
    () => effect,
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.SKYFORGE_EXPERIMENTAL_DISABLE_FILEWATCHER
        else process.env.SKYFORGE_EXPERIMENTAL_DISABLE_FILEWATCHER = previous
      }),
  )
}
