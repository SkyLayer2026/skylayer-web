    ...
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspace.id}&keep=yes`).pipe(
        HttpClientRequest.setHeaders({
          "x-skyforge-directory": "/secret/path",
          "x-skyforge-workspace": "internal",
        }),
      ...
      expect(forwarded?.headers["x-skyforge-directory"]).toBeUndefined()
      expect(forwarded?.headers["x-skyforge-workspace"]).toBeUndefined()
    }),
  )
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const headerResponse = yield* HttpClientRequest.get("/probe").pipe(
        HttpClientRequest.setHeader("x-skyforge-directory", headerDir),
        HttpClient.execute,
      )
