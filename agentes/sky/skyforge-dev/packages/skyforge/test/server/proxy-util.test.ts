...
describe("ProxyUtil", () => {
  ...
  describe("headers", () => {
    ...
    test("strips skyforge-specific headers", () => {
      const req = new Request("http://localhost", {
        headers: {
          "x-skyforge-directory": "/home/user/project",
      ...
      const result = ProxyUtil.headers(req)
      expect(result.get("x-skyforge-directory")).toBeNull()
      expect(result.get("x-skyforge-workspace")).toBeNull()
