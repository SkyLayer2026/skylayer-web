...
import type { QuestionRequest } from "@skyforge/sdk/v2"
import { OpencodeKeymapProvider, registerOpencodeKeymap } from "@skyforge/tui/keymap"
import {
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "skyforge", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("skyforge")
