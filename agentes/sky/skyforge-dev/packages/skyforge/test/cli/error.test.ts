...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/skyforge.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/skyforge.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/skyforge.jsonc", dir: ".skyforge", suggestion: "skyforge" },
        expected:
          'Directory ".skyforge" in /tmp/skyforge.jsonc is not valid. Rename the directory to "skyforge" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/skyforge.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/skyforge.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Or check your config (skyforge.json) provider/model names",
    ].join("\n")
