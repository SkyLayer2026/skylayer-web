...
import { Hash } from "@skyforge/core/util/hash"
...
import { ProjectCopy } from "@skyforge/core/project/copy"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const opencodeFile = path.join(tmp, ".git", "skyforge")
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "skyforge")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "skyforge")
      const wrongCache = path.join(parentDir, ".git", "skyforge")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "skyforge")
      const cacheB = path.join(bareB, "skyforge")
      const wrongCache = path.join(parentDir, ".git", "skyforge")
