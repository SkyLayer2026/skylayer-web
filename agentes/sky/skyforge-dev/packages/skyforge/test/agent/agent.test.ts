...
it.instance("plan agent denies edits except .skyforge/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".skyforge/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".skyforge", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.SKYFORGE_TEST_HOME
      process.env.SKYFORGE_TEST_HOME = test.directory
