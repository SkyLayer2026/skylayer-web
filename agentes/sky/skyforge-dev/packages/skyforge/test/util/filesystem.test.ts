...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "skyforge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "skyforge.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "skyforge.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "skyforge.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["skyforge.json", "skyforge.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "skyforge.json"),
        path.join(project, "skyforge.jsonc"),
      ])
    })
  })
