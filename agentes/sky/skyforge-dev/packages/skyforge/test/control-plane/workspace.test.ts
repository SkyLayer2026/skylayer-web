...
import { EventSequenceTable } from "@skyforge/core/event/sql"
...
import { WorkspaceTable } from "@skyforge/core/control-plane/workspace.sql"
...
const originalEnv = {
  OPENCODE_AUTH_CONTENT: process.env.SKYFORGE_AUTH_CONTENT,
  OPENCODE_EXPERIMENTAL_WORKSPACES: process.env.SKYFORGE_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.SKYFORGE_EXPERIMENTAL_WORKSPACES = "true"
})
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.SKYFORGE_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=skyforge-test"
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=skyforge-test")
