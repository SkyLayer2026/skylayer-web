...
import { ModelsDev } from "@skyforge/core/models-dev"
...
import { LLMEvent, LLMResponse } from "@skyforge/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@skyforge/llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.SKYFORGE_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["skyforge", "native", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["skyforge", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["skyforge", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.SKYFORGE_RECORD_CONSOLE_TOKEN && process.env.SKYFORGE_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "OpenCode Zen",
        ...
        api: zenURL(process.env.SKYFORGE_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.SKYFORGE_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.SKYFORGE_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "skyforge.json"),
