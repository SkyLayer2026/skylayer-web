...
- `config?: Partial<Config.Info>` - Write an `skyforge.json` config file
...
- Directories are created in the system temp folder with prefix `skyforge-test-`
...
it.instance("uses the temp directory", () =>
  Effect.gen(function* () {
    ...
    expect(test.directory).toContain("skyforge-test-")
  }),
)
```
