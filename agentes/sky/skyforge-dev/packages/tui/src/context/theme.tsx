  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "skyforge")
        draft.active = typeof active === "string" ? active : "skyforge"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return (platform?.themes?.discover() ?? Promise.resolve({}))
        ...
        .catch(() => setStore("active", "skyforge"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "skyforge")
            return
          }
