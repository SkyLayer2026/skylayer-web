...
import { ConfigProvider } from "@skyforge/core/config/provider"
import { ConfigMigrateV1 } from "@skyforge/core/v1/config/migrate"
...
import { Policy } from "@skyforge/core/policy"
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "skyforge.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "skyforge.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "skyforge.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "skyforge.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "skyforge.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "skyforge.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".skyforge"), { recursive: true })
            await fs.mkdir(path.join(directory, ".skyforge"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "skyforge.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "skyforge.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "skyforge.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "skyforge.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".skyforge", "skyforge.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".skyforge", "skyforge.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".skyforge")),
              AbsolutePath.make(path.join(directory, ".skyforge")),
            ])
