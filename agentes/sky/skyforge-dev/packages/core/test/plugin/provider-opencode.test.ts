...
import { Catalog } from "@skyforge/core/catalog"
...
import { PluginV2 } from "@skyforge/core/plugin"
import { OpencodePlugin } from "@skyforge/core/plugin/provider/opencode"
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* transform((catalog) => {
          const item = provider("skyforge")
          catalog.provider.update(item.id, () => {})
          const paid = model("skyforge", "paid", { cost: cost(1) })
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* transform((catalog) => {
          const item = provider("skyforge")
          catalog.provider.update(item.id, () => {})
          const free = model("skyforge", "free", { cost: cost(0) })
    ...
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* transform((catalog) => {
          const item = provider("skyforge")
          catalog.provider.update(item.id, () => {})
          const outputOnly = model("skyforge", "output-only", { cost: cost(0, 1) })
  ...
  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined, CUSTOM_OPENCODE_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        ...
        const transform = yield* catalog.transform()
        yield* transform((catalog) => {
          const item = provider("skyforge", { env: ["CUSTOM_OPENCODE_API_KEY"] })
  ...
  it.effect("uses auth-enabled providers as credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        const transform = yield* catalog.transform()
        yield* transform((catalog) => {
          const item = provider("skyforge", { enabled: { via: "account", service: "skyforge" } })
          catalog.provider.update(item.id, (draft) => {
