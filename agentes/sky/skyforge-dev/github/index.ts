...
import { createOpencodeClient } from "@skyforge/sdk"
...
async function getAccessToken() {
  ...
  if (isMock()) {
    ...
    const oidcToken = await core.getIDToken("skyforge-github-action")
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/skyforge-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
